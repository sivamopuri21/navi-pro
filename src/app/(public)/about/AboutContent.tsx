    ...
    <>
      ...
      <Section>
        <div className="max-w-3xl mx-auto text-center">
          ...
          <p className="text-gray-600 leading-relaxed">
            NaviPro Projects Private Limited was established with a singular vision — to
            transform the landscape of civil and interior construction in India. Based out
            of Hyderabad, Telangana, we have grown into a trusted partner for hospitals,
            diagnostic laboratories, and large-scale infrastructure projects. Our team
            combines engineering expertise with a passion for quality, delivering projects
      ...
      <Section className="!bg-surface-alt">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {values.map((v, i) => (
            <motion.div key={v.title} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} className="bg-white rounded-2xl p-8 shadow-md text-center">
              <div className="w-14 h-14 mx-auto rounded-xl bg-accent/10 flex items-center justify-center text-accent">
                <v.icon size={28} />
              </div>
              <h3 className="mt-5 font-bold text-xl text-primary">{v.title}</h3>
